...
// Seminarzeitplan https://docs.google.com/spreadsheets/d/1h--_7eA3aqhPg2Wy3DwseZhQo4OsXbjrEuH5MSpabzo/edit?usp=sharing
// Probe Inhaltsverzeichnis https://docs.google.com/document/d/1QqGrMpN0ayflMX5RmnmuAyqDpepebeGIHjoB3F-e1p8/edit?usp=sharing
...
//      Liste von Gegenstaende
        ...
        Gegenstaende.add(new Item(20, 4));
        int Rucksackgroesse = 10;

        System.out.println(Beste(Generationsgenerator(Gegenstaende, 9), Gegenstaende, Rucksackgroesse, 20));
 ...
 * */
    ...
    public static String Beste(List<int[]> Generation, List<Item> Gegenstaende, int Rucksackgroesse, int AnzahlderGenerationen) {
        List<int[]> Genus = Generation;
//      "Neue" Generation wird erstellt
        for (int i = 0; i < AnzahlderGenerationen; i++) {
            Genus = NextGen(Genus, Gegenstaende, Rucksackgroesse);
        }
//      Beste Kombi der Liste
        return Arrays.deepToString(Bestekombi(Genus, Gegenstaende, Rucksackgroesse).toArray());
    }


    // Methode erstellt moegliche Packombinationen
    public static List<int[]> Generationsgenerator(List<Item> Gegenstaende, int AnzahlParents) {
        List<int[]> Generation = new ArrayList<>();
        int[] Parent;

        // Die Anzahl der "Parents" darf nicht kleiner als 0 sein
        if (AnzahlParents == 0) {
            AnzahlParents = 2;
        }

        // Die moeglichen Kombinationen werden erstellt hierfuer sind die Anzahl der "Items" und die Anzahl der "Parents" wichtig

        for (int i = 0; i < AnzahlParents; i++) {

            Parent = new int[Gegenstaende.size()];

            // Fuer jedes Parrent wird zufaellig entschieden, ob ein "Item" enthalten ist
            for (int j = 0; j < Gegenstaende.size(); j++) {

                Parent[j] = new Random().nextInt(2);
            }
            //Diese "Parents" werden zu einer Generation zusammengefasst und spaeter ausgegeben
...
//      Die zwei besten Kombinationen werden zurueckgegeben
    ...
    public static List<int[]> Bestekombi(List<int[]> Gen, List<Item> Gegenstaende, int Rucksackgroesse) {
        int value;
...
//      Beste Kombination wird zurueckgeben
    ...
    //  Methode gibt die groesste Zahl in einem Array zurueck
    ...
    //  Methode gibt die Zweigroesstenwerte eines Arrays zurueck
